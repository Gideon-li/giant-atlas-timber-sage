...
${_([`门`,`基础分（校准后）`,`事项偏置`,`合计`],C(s,e.gateBias,r))}

**九星偏置**

${_([`星`,`基础分（校准后）`,`事项偏置`,`合计`],C(o,e.starBias,i))}

**八神偏置**

${_([`神`,`基础分（校准后）`,`事项偏置`,`合计`],C(t,e.godBias,a))}

特殊规则：${e.id===`health`?`病星天芮空亡或入墓 +12（病气衰减）；天芮帝旺 −10；生门 +8；死门非空 −8。填写出生年则用神宫改取年干天盘宫。`:e.id===`romance`?`沐浴改作 +8（桃花）；男测兑/坤宫 +6，女测乾/坎宫 +6。`:e.id===`job`||e.id===`career`?`若填写出生年，用神宫改取年干在天盘所落宫（以年命代值符/开门宫）。`:`无额外翻转规则。`}`}).join(`

`)}function se(){let e=a(S),t=m(e),n=u(e,`wealth`),r=_([`事项`,`用神宫`,`神/星/门`,`开始分`,`过程分`,`收局分`,`综合 S`,`百分比`,`总断`],t.map(t=>{let n=e.palaces[t.palaceId];return[t.name,`${n.bagua}${t.palaceId}`,`${n.god??`—`}/${n.star}/${n.gate??`—`}`,String(t.phases.start.score),String(t.phases.process.score),String(t.phases.end.score),(t.score>0?`+`:``)+String(t.score),`${t.probability}%`,t.level]})),i=_([`阶段`,`因子`,`说明`,`分值`],n.factors.map(e=>[e.phase,e.label,e.detail,(e.weight>0?`+`:``)+String(e.weight)])),o=n.factors.filter(e=>e.phase===`start`).reduce((e,t)=>e+t.weight,0),s=n.factors.filter(e=>e.phase===`process`).reduce((e,t)=>e+t.weight,0),c=n.factors.filter(e=>e.phase===`end`).reduce((e,t)=>e+t.weight,0),l=n.factors.filter(e=>e.phase===`aux`).reduce((e,t)=>e+t.weight,0);return{chart:e,ranked:r,wealth:n,factors:i,mix:{start:o,process:s,end:c,aux:l,raw:o*x.start+s*x.process+c*x.end+l*x.aux,score:n.score,p:n.probability}}}var w=se(),T={title:`十二类日常事项加性分值模型`,method:`拆补法转盘排盘后，按事项取用神宫，神/星/门基础分（天气校准后）加事项偏置，再加干支、长生、月令、格局；四阶段加权得 S，P=σ(S/22)。`,scoreScale:22,probability:`P = clip(sigmoid(S/22), 0.04, 0.96)，界面为百分数`,phaseWeights:x,luckBands:{大吉:`S≥42`,吉:`20≤S<42`,小吉:`6≤S<20`,平:`-6<S<6`,小凶:`-20<S≤-6`,凶:`-42<S≤-20`,大凶:`S≤-42`},bases:{god:{...t},gate:{...s},star:{...o},stem:{...d},changsheng:{...r},calibration:{method:e.method,globalScale:e.globalScale,meanXunAcc:e.meanXunAcc}},events:n.map(e=>({id:e.id,name:e.name,brief:e.brief,yongShen:e.yongShen,target:e.target,secondary:e.secondary??null,gateBias:e.gateBias,starBias:e.starBias,godBias:e.godBias})),patterns:y,stemGe:Object.entries(i).map(([e,t])=>({stems:e,...t})),sampleCivil:S,sampleJu:w.chart.ju.label,sampleEvents:m(w.chart).map(e=>({id:e.eventId,name:e.name,palaceId:e.palaceId,score:e.score,probability:e.probability,level:e.level,phases:e.phases,topFactors:e.factors.slice(0,8).map(e=>({phase:e.phase,label:e.label,detail:e.detail,weight:e.weight}))}))};function ce(){return _([`事项`,`用神种类`,`主用神`,`次看`,`取宫规则`],n.map(e=>[e.name,e.yongShen===`zhifu`?`值符宫`:e.yongShen===`gate`?`八门`:e.yongShen===`star`?`九星`:`八神`,e.target,e.secondary?`${e.secondary.name}`:`—`,e.yongShen===`zhifu`?`值符飞宫；填出生年则改年干天盘宫`:`找「${e.target}」所在宫；求职/事业/健康填年命则改年干天盘宫`]))}function le(){let{chart:e,ranked:n,wealth:a,factors:u,mix:p}=w,m=e.palaces[a.palaceId];return`
...
${_([`条件`,`总断`],[[`S ≥ 42`,`大吉`],[`20 ≤ S < 42`,`吉`],[`6 ≤ S < 20`,`小吉`],[`−6 < S < 6`,`平`],[`−20 < S ≤ −6`,`小凶`],[`−42 < S ≤ −20`,`凶`],[`S ≤ −42`,`大凶`]])}
...
${v(`天盘干 STEM_BASE（乙丙丁为三奇）`,d)}

${v(`十二长生 CHANGSHENG_SCORE（临用神宫时写入过程）`,r)}
...
天干五合：${Object.entries(ee).filter((e,t)=>t%2==0).map(([e,t])=>`${e}${t}`).join(`、`)}。天干相冲：${Object.entries(ne).filter((e,t)=>t%2==0).map(([e,t])=>`${e}${t}`).join(`、`)}。
...
五行：门 ${Object.entries(c).map(([e,t])=>`${e}=${t}`).join(`，`)}；星 ${Object.entries(ie).map(([e,t])=>`${e}=${t}`).join(`，`)}。
...
${_([`格局`,`分值`,`触发`,`文献`],y.map(e=>[e.name,String(e.weight),e.when,e.source]))}

十干克应键（天盘+地盘）数值备份：

${_([`天盘+地盘`,`格局`,`分值`,`断语`],Object.entries(i).map(([e,t])=>[e,t.name,String(t.weight),t.detail]))}
...
${_([`活动`,`宜门（+16）`,`忌门（−16）`],te.map(e=>[e.name,e.prefer.join(`、`),e.avoid.join(`、`)]))}

八门古法宜忌（《烟波钓叟赋》门旨）：

${_([`门`,`宜`,`忌`,`歌诀摘要`],Object.entries(l).map(([e,t])=>[e,t.suit.join(`、`),t.avoid.join(`、`),t.classic]))}
...
## 第 11 章 数值算例（${S.year}-${String(S.month).padStart(2,`0`)}-${String(S.day).padStart(2,`0`)} ${String(S.hour).padStart(2,`0`)}:${String(S.minute).padStart(2,`0`)} 北京时间）

排盘结果：${e.ju.label}。日柱 ${e.pillars.day.name}，时柱 ${e.pillars.hour.name}。值符在 ${e.meta.zhiFuPalace} 宫，值使 ${e.meta.zhiShiGate}。
...
用神：生门所落 ${m.bagua}${a.palaceId} 宫（${m.direction}）。临 ${m.god??`无神`} / ${m.star} / ${m.gate??`无门`}，天盘 ${m.heavenStem}，地盘 ${m.earthStem}。总断 ${a.level}，S=${a.score>0?`+`:``}${a.score}，顺利倾向 ${a.probability}%。

全部因子（即界面「权重明细」的完整导出）：

${u}

阶段合计：始 ${g(p.start)}，中 ${g(p.process)}，终 ${g(p.end)}，辅 ${g(p.aux)}。

加权：

\\\\[ S_{raw}=0.25\\times ${g(p.start)}+0.35\\times ${g(p.process)}+0.40\\times ${g(p.end)}+0.55\\times ${g(p.aux)}=${g(p.raw)} \\\\]

四舍五入并截断到 [−100,100] 得 S=${p.score}。百分比

\\\\[ P=\\mathrm{round}(100\\cdot\\sigma(${p.score}/22))=${p.p}\\% \\\\]

总断档 ${f(p.score)}，与 probabilityOf(${p.score})=${re(p.score)}% 一致。
...
`}var E=p,D=h;function O(e){return`${(e*100).toFixed(1)}%`}function k(e){return e.toFixed(4)}function A(e){return e.toFixed(3)}function j(e,t){return[`| ${e.join(` | `)} |`,`|${e.map(()=>`---`).join(`|`)}|`,...t.map(e=>`| ${e.join(` | `)} |`)].join(`
`)}function ue(){return j([`区`,`气候`,`样本日`,`训练`,`检验`,`雨日`,`雨日比`,`有雨训练`,`有雨检验`,`旬训练`,`旬检验`,`截距分值`],E.regions.map(e=>{let t=e.metrics;return[e.name,e.climate,String(e.n),String(e.trainN),String(e.testN),String(e.rainDays),O(e.rainRate),O(t.rainAccTrain),O(t.rainAccTest),O(t.xunAccTrain),O(t.xunAccTest),A(t.interceptScore)]}))}function M(e){let t=[[`截距 b`,k(e.scoreModel.b),A(e.metrics.interceptScore),`无对应特征，吸收该区气候雨日基线`]];for(let n=0;n<E.featureNames.length;n++){let r=e.scoreModel.w[n]??0;t.push([E.featureNames[n],k(r),A(r*22),N(E.featureNames[n],r)])}return j([`特征`,`logit β`,`分值 22β`,`产生方式`],t)}function N(e,t){return e===`星_天禽`?`坎宫不出现天禽（寄坤宫），该列恒为 0，梯度为 0`:Math.abs(t)<1e-8?`训练后近 0`:e.startsWith(`神_`)||e.startsWith(`门_`)||e.startsWith(`星_`)?`坎宫 one-hot=1 时对该区 logit 的加项；由 1827 日交叉熵梯度下降得到`:e===`阴遁`?`阴遁日取 1，阳遁取 0`:e===`伏吟`||e===`反吟`?`盘面格局 0/1`:e===`坎空`?`坎宫旬空为 1`:e===`年积日sin`||e===`年积日cos`?`节气气候学三角项，非奇门符号`:`逻辑回归系数`}function de(){return E.regions.map((e,t)=>{let n=e.metrics,r=[...e.allFactors].slice(0,5).map(e=>`${e.name} ${e.score>=0?`+`:``}${A(e.score)}`).join(`；`),i=e.daily3.b.map((t,n)=>`${e.daily3.classes[n]}=${k(t)}`).join(`，`);return`### 6.10.${t+1} 对照 ${e.name}（${e.place}）

气候带：${e.climate}。样本 ${e.n} 日（训练 ${e.trainN} = ${E.start}–${E.trainUntil}；检验 ${e.testN} = ${E.testFrom}–${E.end}）。雨日 ${e.rainDays}，雨日比 ${O(e.rainRate)}。

主模型（Bernoulli 有雨）准确率：训练 ${O(n.rainAccTrain)}，检验 ${O(n.rainAccTest)}。旬阴晴：训练 ${O(n.xunAccTrain)}，检验 ${O(n.xunAccTest)}。辅模型 softmax 晴/阴/雨：训练 ${O(n.dailyAccTrain)}，检验 ${O(n.dailyAccTest)}。

分值最大的五项（S=22β）：${r}。

Softmax 三类截距：${i}。主模型截距 b=${k(e.scoreModel.b)}，对应分值 ${A(n.interceptScore)}。

该区完整 31 维权重如下。每一行的 β 都是在该区 1827 个训练日上，以交叉熵 + L2 全批梯度下降 120 轮得到的最终值；分值列与事项预测同一单位。

${M(e)}`}).join(`

`)}function P(){let t=e.pooledLogit.map(e=>Math.abs(e.logit)).sort((e,t)=>e-t);return t[Math.floor(t.length/2)]||.01}function F(e,t){return Math.max(.55,Math.min(1.35,.75+.5*(Math.abs(e)/(t*3+1e-6))))}function I(t,n){return e.pooledLogit.find(e=>e.name===`${t}_${n}`)?.logit??0}function L(t,n,r,i){let a=P(),o=e.globalScale;return`### ${t}\n\n${j([`名`,`经典先验 w0`,`全国区县平均 β`,`分值 22β`,`信度 r`,`尺度 g`,`w0·r·g`,`四舍五入后`],Object.keys(r).map(e=>{let t=r[e],s=I(n,e),c=F(s,a),l=t*c*o;return[e,String(t),k(s),A(s*22),A(c),A(o),A(l),String(i[e])]}))}`}function R(){let t=P(),n=e.globalScale;return[{name:`死门`,prefix:`门`,classic:e.classicGate.死门,now:e.gate.死门,why:`刘伯温以死门为凶门。天气回归里该 one-hot 的 |β| 相对中位数偏大，故幅度加大，符号仍为负。`},{name:`生门`,prefix:`门`,classic:e.classicGate.生门,now:e.gate.生门,why:`生门为吉门。天气 |β| 未显著高于中位数，乘 g 后略降，避免把雨日气候学误写成求财加分。`},{name:`开门`,prefix:`门`,classic:e.classicGate.开门,now:e.gate.开门,why:`开门人事主官贵。校准后仍为 +20，尺度被 g 与 r 抵消。`},{name:`值符`,prefix:`神`,classic:e.classicGod.值符,now:e.god.值符,why:`值符为八神之首。天气坎宫上该列 |β| 偏小，信度下调。`},{name:`玄武`,prefix:`神`,classic:e.classicGod.玄武,now:e.god.玄武,why:`古法玄武主雨。人事仍作凶神，不把雨势正号抄到求财。`},{name:`天芮`,prefix:`星`,classic:e.classicStar.天芮,now:e.star.天芮,why:`天芮寄死门宫。天气 |β| 与死门共线，人事凶星幅度加大。`}].map((r,i)=>{let a=I(r.prefix,r.name),o=F(a,t),s=r.classic*o*n;return`例 ${i+1}　${r.name}

- 经典先验（刘伯温人事吉凶）w0 = ${r.classic}。
- 全国区县 Bernoulli 系数算术平均 β̄ = ${k(a)}，对应分值 22β̄ = ${A(a*22)}。
- 中位数 |β|med = ${k(t)}。
- 信度 r = clip(0.75 + 0.5 × |β̄| / (3 med), 0.55, 1.35) = ${A(o)}。
- 全国旬准确率尺度 g = 0.92 + 0.16 × ${A(e.meanXunAcc)} = ${A(n)}。
- 乘积 w0 × r × g = ${r.classic} × ${A(o)} × ${A(n)} = ${A(s)}。
- 四舍五入得最终事项权重 ${r.now}。${r.why}`}).join(`

`)}function z(){let e=[`神_玄武`,`神_九天`,`门_死门`,`门_生门`,`门_开门`,`星_天芮`,`阴遁`,`年积日cos`];return j([`区`,`截距分`,...e],E.regions.map(t=>{let n=[t.name,A(t.metrics.interceptScore)];for(let r of e){let e=E.featureNames.indexOf(r),i=(t.scoreModel.w[e]??0)*22;n.push(A(i))}return n}))}var B=E.regions.find(e=>e.id===`guangzhou`),V=E.regions.find(e=>e.id===`haikou`);E.regions.find(e=>e.id===`ouhai`);var H=P(),fe=D.samples.find(e=>e.code===`330304`),pe=D.samples.find(e=>e.code===`110108`),me=D.samples.find(e=>e.code===`440106`);function U(){return j([`省`,`区县数`,`雨日比`,`有雨检验`,`旬检验`],D.provinceMetrics.map(e=>[e.province,String(e.n),O(e.rainRate),O(e.rainAccTest),O(e.xunAccTest)]))}function he(e){let t=[[`截距 b`,k(e.scoreModel.b),A(e.metrics.interceptScore),`吸收该区雨日基线`]];for(let n=0;n<D.featureNames.length;n++){let r=e.scoreModel.w[n]??0;t.push([D.featureNames[n],k(r),A(r*22),N(D.featureNames[n],r)])}return j([`特征`,`logit β`,`分值 22β`,`产生方式`],t)}function ge(){return D.samples.map((e,t)=>{let n=e.metrics,r=e.daily3.b.map((t,n)=>`${e.daily3.classes[n]}=${k(t)}`).join(`，`);return`### 6.${t+1} ${e.province}${e.city}${e.name}（${e.code}）

中心 ${e.lat.toFixed(4)}°N ${e.lng.toFixed(4)}°E。样本 ${D.nDays} 日（训练 ${D.nTrainDays} = ${D.start}–${D.trainUntil}；检验 ${D.nTestDays} = ${D.testFrom}–${D.end}）。雨日 ${e.rainDays}，雨日比 ${O(e.rainRate)}。

主模型有雨准确率：训练 ${O(n.rainAccTrain)}，检验 ${O(n.rainAccTest)}。旬阴晴：训练 ${O(n.xunAccTrain)}，检验 ${O(n.xunAccTest)}。softmax 晴/阴/雨：训练 ${O(n.dailyAccTrain)}，检验 ${O(n.dailyAccTest)}。

Softmax 三类截距：${r}。主模型截距 b=${k(e.scoreModel.b)}，对应分值 ${A(n.interceptScore)}。该区 31 维权重由本区 ${D.nTrainDays} 个训练日交叉熵 + L2 全批梯度下降 ${D.ml.epochs} 轮得到，不与邻区共享。
...
- 全国区县旬准确率均值如实报告为 ${O(D.meanXunAcc)}，有雨日值检验 ${O(D.meanRainAccTest)}；不虚报 90%。十二气候区对照中广州旬检验 ${O(B.metrics.xunAccTest)}、海口旬训练 ${O(V.metrics.xunAccTrain)} 仍可单独查看。
...
x 为 31 维，顺序固定为：${E.featureNames.join(`，`)}。
...
有雨标签 y∈{0,1}，p=σ(b+w⊤x)，L2 系数 λ=${E.ml.l2}：
...
- 轮数 epochs = ${E.ml.epochs}。
- 学习率 η = ${E.ml.learningRate}。
- L2 λ = ${E.ml.l2}，不惩罚截距 b。
- 初始化：w=0，b=0。
- 更新：
...
十二区各自独立跑上述循环，不共享 w（截距必须吸收当地雨日基线，例如海口雨日比 ${O(V.rainRate)}，哈尔滨 ${O(E.regions[0].rainRate)}）。全国区县训练把同一循环并行到 ${D.nDistricts} 个 y 向量：X 仍是 ${D.nTrainDays}×31，Y 是 ${D.nTrainDays}×${D.nDistricts}，一次矩阵梯度同时更新所有区县的 w、b。
...
全国旬准确率均值 ${O(D.meanXunAcc)}，有雨日值检验均值 ${O(D.meanRainAccTest)}。不把任何省份改写成 90%。最强特征普遍是年积日 cos（气候学季节项），奇门 one-hot 提供区内增量。
...
${j([`特征`,`平均 logit β̄`,`分值 22β̄`],D.pooledLogit.map(e=>[e.name,k(e.logit),A(e.score)]))}

### 6.10 十二气候区对照（Open-Meteo / ERA5）

以下十二区不是运行时天气模型，仅保留为方法对照。运行时选点浙江省温州市瓯海区与北京市海淀区使用各自区县权重，不再落入同一「华东」或「华北」模型。

全国对照旬均值 ${O(E.eventCalibration.meanXunAcc)}。广州旬检验 ${O(B.metrics.xunAccTest)}、海口旬训练 ${O(V.metrics.xunAccTrain)}。
...
中位数 med = median_k |β̄_k| = ${k(H)}。信度

\\\\[ r_k = \\mathrm{clip}\\Big(0.75 + 0.5\\frac{|\\bar\\beta_k|}{3\\,\\mathrm{med}},\\ 0.55,\\ 1.35\\Big) \\\\]

全国旬准确率尺度

\\\\[ g = 0.92 + 0.16\\,\\bar a_{旬} = 0.92 + 0.16\\times ${k(e.meanXunAcc)} = ${k(e.globalScale)} \\\\]
...
${L(`八神：从经典到天气校准`,`神`,e.classicGod,e.god)}

${L(`八门：从经典到天气校准`,`门`,e.classicGate,e.gate)}

${L(`九星：从经典到天气校准`,`星`,e.classicStar,e.star)}
...
${j([`类`,`名`,`最终权重`],[...Object.entries(e.god).map(([e,t])=>[`八神`,e,String(t)]),...Object.entries(e.gate).map(([e,t])=>[`八门`,e,String(t)]),...Object.entries(e.star).map(([e,t])=>[`九星`,e,String(t)])])}

事项百分比仍为 P=σ(S/22)，与天气同一变换。例如求财遇生门 ${e.gate.生门} 分、死门 ${e.gate.死门} 分，再加事项偏置、星神与格局辅项后按第 8 章加权过 sigmoid。
...
5. 全国旬准确率均值 ${O(D.meanXunAcc)}，有雨日值检验 ${O(D.meanRainAccTest)}，本文不虚报 90%。
...
EVENT_MODEL_SPEC 含：scoreScale=${T.scoreScale}；phaseWeights 始/中/终/辅 = ${T.phaseWeights.start}/${T.phaseWeights.process}/${T.phaseWeights.end}/${T.phaseWeights.aux}；events ${T.events.length} 类；patterns ${T.patterns.length} 条；sampleJu「${T.sampleJu}」。下载的 JSON 与论文表格同一数据源，避免手抄误差。
